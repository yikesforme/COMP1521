...
clear_loop:
		bge   $s0, $t7, end_clear   # row = 0
		li    $s1, 0                # col = 0
clear_loop1:
		bge   $s1, $t2, end_clear1  # col < NCOLS
...
draw_loop1:
		bge   $s0, $t0, end_draw1   # row = 0 row < NROWS
		li    $s1, 0                # col = 0
draw_loop2:
		bge   $s1, $t2, end_draw2
...
init_loop:
  bge   $t1, $a2, end_init      # nsegs < len
  beq   $t0, $t4, end_init      # newCol !=NCOLS
...
### TODO: complete this function
	...
	# code for function
	blt    $a0, 0, endGird         # col >= 0
	bge    $a0, 40, endGird        # col < NCOLS
	blt    $a1, 0, endGird         # row >= 0
	bge    $a1, 20, endGird        # row < NROWS
...
# if any condition not correct, return 0
endGird:
	li    $v0, 0
	# tear down stack frame
	lw	  $ra, -4($fp)
...
for:
	bge   $t6, $a2, end_over                # i <len
	mul   $s2, $s0, $t6
	lw    $s3, wormCol($s2)                 # s3 = wormCol[i]
	lw    $s4, wormRow($s2)                 # s4 = wormRow[i]

	beq   $a0, $s3, check_again             # col == wormCol[i]
	addi  $t6, $t6, 1                       # i ++
	j     for                               # start loop
...
move_if:
	...
	bne   $t2, 1, end_if            # onGrid(col, row) == 1
	bne   $t3, 0, end_if            # overlaps(col, row, len) == 0
	mul   $t6, $s7, $t4             # position
